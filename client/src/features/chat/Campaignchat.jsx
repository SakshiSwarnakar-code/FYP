...
const Spinner = ({ size = "w-5 h-5" }) => (
  <div
    className={`${size} border-2 border-sky-400 border-t-transparent rounded-full animate-spin`}
  />
);
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {filtered.map((c) => {
          ...
          const lastTxt =
            c.lastMessage?.text ||
            (c.lastMessage?.attachment ? "Sent an attachment" : "Say hello 👋");
          const lastTime = c.lastMessage?.createdAt
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between gap-1 mt-0.5">
                  ...
                  {(c.unreadCount ?? 0) > 0 && (
                    <span
                      className="flex-shrink-0 min-w-[18px] h-[18px] px-1 rounded-full bg-sky-500
...
const PeopleTab = ({
  ...
  campaigns.forEach((camp) => {
    ...
    const adminId =
      typeof camp.createdBy === "object" ? camp.createdBy?._id : camp.createdBy;
  ...
  const grouped = filtered.reduce((acc, e) => {
    if (!acc[e.campaignId])
      acc[e.campaignId] = { title: e.campaignTitle, items: [] };
  ...
  const findConvo = (personId) =>
    conversations.find((c) =>
      c.participants?.some(
        (p) => p && typeof p === "object" && p._id === personId
      )
    );
        ...
        {allEntries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center gap-2">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Volunteers must be accepted into a campaign before they appear
              here.
            </p>
          </div>
        )}
        ...
        {Object.entries(grouped).map(([campId, group]) => (
          <div key={campId}>
            ...
            {group.items.map((entry) => {
              const isOnline = !!(
                entry.personId && onlineUsers.has(entry.personId)
              );
                ...
                >
                  ...
                  <div className="flex-shrink-0 w-10 flex items-center justify-center">
                    {existingConvo ? (
                      ...
                      <div
                        className="w-8 h-8 rounded-full bg-sky-500 flex items-center justify-center
...
export default function CampaignChat({ currentUser }) {
  ...
  // Keep refs so socket handlers can safely work even before initial loads finish.
  ...
  const activeConvo =
    conversations.find((c) => c._id === activeConvoId) ?? null;
  ...
  useEffect(() => {
    ...
    socket.on("message_received", ({ message }) => {
      const isActiveConversation =
        message.conversationId === activeConvoIdRef.current;
    ...
    socket.on("new_message_notification", ({ conversationId, message }) => {
      const convoId = conversationId?.toString?.() ?? conversationId;
      const activeId =
        activeConvoIdRef.current?.toString?.() ?? activeConvoIdRef.current;

      setConversations((prev) => {
        // If conversations haven't loaded into state yet for some reason, fall back to last ref.
        const base =
          Array.isArray(prev) && prev.length
            ? prev
            : conversationsRef.current ?? prev;
        const idx = base.findIndex(
          (c) => (c?._id?.toString?.() ?? c?._id) === convoId
        );
        if (idx === -1) return prev;

        const convo = base[idx];
        const isActiveConversation = convoId === activeId;
        const updated = {
          ...convo,
          // Sidebar preview should update from socket payload even when chat isn't open.
  ...
  const chatOther = (() => {
    if (pendingChat) {
      return {
        ...
        firstName:
          pendingChat.personObj?.firstName ??
          pendingChat.personName?.split(" ")[0] ??
          "",
        lastName:
          pendingChat.personObj?.lastName ??
          pendingChat.personName?.split(" ")[1] ??
          "",
      };
    }
    if (activeConvo) {
      return (
        activeConvo.participants?.find(
          (p) =>
            p && typeof p === "object" && p._id && p._id !== currentUser._id
        ) ?? null
      );
    }
    return null;
                ...
                {chatOther ? (
                  <>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p
                        className={`text-xs ${
                          chatOtherOnline ? "text-emerald-500" : "text-gray-400"
                        }`}
                      >
                ...
                {Object.entries(groupedMessages).map(([date, msgs]) => (
                  <div key={date}>
                    ...
                    {msgs.map((m, i) => {
                      ...
                      const seen =
                        isMine &&
                        (m.readBy ?? []).some((id) => id !== currentUser._id);
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <div
                              className={`max-w-full px-3.5 py-2.5 rounded-2xl text-[13.5px] leading-relaxed shadow-sm
                                ${
                                  isMine
                          ...
                          {isMine && isLast && (
                            <p
                              className={`text-[11px] mt-0.5 mr-1 ${
                                seen ? "text-emerald-500" : "text-gray-400"
                              }`}
                            >
                              {m.pending
                                ? "Sending…"
